 ...
 * 两个分拨中心到彼此的单量对等，则可以开双边车（即同一辆车可以往返对开），而当两个中心的对发单量不对等时，
 * 则会采用单边车，并且双边车的成本是低于单边车的，即将两辆对开的单边车合并为一辆往返的双边车是能够节省运力成本的
 * <p>
 * 单边车优化原则：
 * <p>
 ...
 * 问题如下：
 * <p>
 * 以某快递公司的实际单边车数据为例（线路ID编码;出分拨中心; 出分拨中心所在省;到达分拨中心;到达分拨中心所在省；
 * 车型；），进行优化，优化的规则参照以上，并且优先级依次降低，合并的时候需要考虑车型（分为17.5m和9.6m两种）：
 * 1、相同车型才能进行合并；
 * 2、两辆同方向的9.6m可以与一辆17.5m的对开车型合并优化 说明：优化输出结果按照规则分类，
...
//                UnilateralLine ui = lineList.get(i);
//                UnilateralLine uj = lineList.get(j);
//                if (ui.getEPro().equals(uj.getSPro()) && ui.getSPro().equals(uj.getEPro()) && ui.getTType().equals(uj.getTType())) {
//                    result.add("rule1:" + ui.getId() + "+" + uj.getId());
//                } else if (ui.getEPro().equals(uj.getSPro()) && ui.getTType().equals(uj.getTType())) {
//                    for (int k = j; k < lineList.size(); k++) {
//                        UnilateralLine uk = lineList.get(k);
//                        if (uj.getEPro().equals(uk.getSPro()) && uk.getSPro().equals(ui.getEPro()) && uk.getTType().equals(uj.getTType())) {
//                            result.add("rule2:" + ui.getId() + "+" + uj.getId() + "+" + uk.getId());
//                        }
//                    }
//                }
//            }
